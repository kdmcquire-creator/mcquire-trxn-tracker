...
- [ ] **AT&T still mis-excluded (separate fix).** `excl-004` no longer grabs it, but AT&T's `exact:'at&t'` rules (`llc-011`/`p10-024`/`p10-025`) don't match `"ATT* BILL PAYMENT"`, so ~$2–4k of AT&T stays in `Exclude`. Needs those rules broadened to `contains` **and** the 832-687-0468 business-vs-personal line split decided before auto-classifying (the large bills already flag `split_flag`). Judgment call — leave for Kyle.
