...
**Last updated:** 2026-06-22 · **Stack:** Electron 28 + React 18 + TypeScript 5 + Tailwind 3 + sql.js (WASM SQLite), built with electron-vite + electron-builder → Windows NSIS installer.
...
- **2026-06-22 (USAA statement PDF import — 1.0.9):** native import of USAA checking-statement PDFs (digital text — `pdf-parse` extracts cleanly). **Parser** `electron/services/usaa-statement-parser.ts` (pure) → the CSV path's `ParsedRow[]` (sign: **positive = expense**); **validated to the cent** vs a real statement (53 txns; debits/credits/ending reconcile to the **Activity Summary** — the built-in correctness check). Handles credit rows (`0` then `$amt $bal` on one line), multi-line descriptions, a description with a stray `$`, the **check row** (amount on the date line: `CHECK # 1794 $200.00` then `0`/`$bal` — recover the debit from the text), and the **Dec→Jan year rollover**. **Same-account fuzzy dedup** (`dedup.ts matchStatementDuplicates`: exact amount + ≤2 days + a shared significant description token, **count-aware**) — the hash + `isCrossAccountDuplicate` can't catch a re-import into the SAME account with differently-formatted descriptions. **Ingest** in `HistoricalImportService` (`previewUSAAStatement`/`importUSAAStatement`): resolve account by mask, dedup, classify, insert; transfers/payments → **Exclude** (Kyle's choice). Added **9 exclude rules** `excl-010..018` (zelle/funds transfer/extrl xfer/ext trnsfr/wire transfer/crd autopay/gsbank/cashout/online pmt) so USAA's abbreviated transfers actually auto-exclude (plain "payment"/"transfer" missed them). IPC `import:usaa-pdf-{pick,preview,run}` + `importBridge` + **Import Statement** screen (drag-drop + preview-then-confirm). End-to-end vs live DB: the real statement = **8 dups / 19 expenses / 26 transfers excluded**. **KEY finding: the PDF is comprehensive (all transactions) but the existing 8178 data is expense-filtered (Monarch/CSV dropped transfers)** — so the importer maps to the same 8178 account + same-account dedup, and classifies transfers to Exclude. Tests: `usaa-statement-parser`, `statement-dedup`, `usaa-statement-import`.
